<x-dashboard-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        <div class="flex items-center justify-between mb-6">
            ...
            <!-- Botón Volver -->
            <div class="mt-8">
                <a href="{{ route('revisar_usuario_examen', ['usuario' => $usuario]) }}" 
        ...
        @if($examenRealizado && !$examenRealizado->calificacion)
        ...
        <!-- Lista de Preguntas -->
        <form action="{{ route('examen_realizado.update', ['usuario' => $usuario, 'examen' =>$examenRealizado->examen_id]) }}" method="POST">        
        @csrf
        @method('PUT')
                    ...
                    <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                <div class="mt-3">
                                    ...
                                    @if($esAbierta && $noCalificada && $respuesta)
                                        <div class="mt-2">
                                            <label for="calificacion_{{ $pregunta->idPregunta }}" class="block text-sm font-medium text-gray-700">Califica (0 a {{ $pregunta->puntos }}):</label>
                                            <input type="number" name="calificaciones[{{ $respuesta->id }}]" id="calificacion_{{ $pregunta->idPregunta }}"
                                                ...
                                                min="0" max="{{ $pregunta->puntos }}" step="0.5" required>
                                        </div>
                                    @endif
                                </div>
                            </div>
